...
public class GenericMemoryRangeTableProvider : IRangeTableProvider
{
    ...
    public List<RangeTable> FindTables()
    {
        ...
        for (int i = rangeTables.Count - 1; i >= 0; i--)
        {
            ...
            {
                Console.WriteLine($"Table {table.StartAddress:x8} has invalid entries (End : {table.EndAddress:x8})");
        ...
        while (dataSection.Length >= RangeTable.EntrySize + 8)
        {
            ...
            if (candidates.Count != 0)
            {
                for (int i = 0; i < candidates.Count; i++)
                {
                    ...
                    if ((dataAddress - table.StartAddress) % RangeTable.EntrySize == 0)
                    {
            ...
            else
            {
                if (rangeSize2 != 0)
                {
                    ...
                    if (!addressOutOfRange && !lengthOutOfRange)
                    {
                        candidates.Add(new RangeTable(dataAddress, dataAddress + RangeTable.EntrySize * 2, Library));
                    }
                }
            }
