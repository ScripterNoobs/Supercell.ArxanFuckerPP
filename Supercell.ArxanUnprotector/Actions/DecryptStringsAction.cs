...
public class DecryptStringsAction : IAction
{
	public string Execute(Library original, Library modified, string output, bool isKillStringEncryption)
	{
		...
		if (original != null && modified != null)
		{
			// Perform dump-diff decryption
			Console.WriteLine("[*] Performing dump-diff static decryption...");
			HashSet<int> relocs = original.GetRelocationOffsets();
			int memorySize = original.MemorySize;
			Span<byte> origMem = original.Take(0, memorySize);
			Span<byte> dumpMem = modified.Take(0, memorySize);

			int copiedBytes = 0;
			for (int i = 0; i < memorySize; i++)
			{
				if (origMem[i] != dumpMem[i])
				{
			...
			Console.WriteLine($"[*] Copied {copiedBytes} decrypted bytes from dump (relocations filtered).");

			if (isKillStringEncryption)
			{
				original.KillStringsEncryption();
			}
